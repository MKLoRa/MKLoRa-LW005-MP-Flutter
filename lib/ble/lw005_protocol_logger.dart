...
class Lw005ProtocolLogger {
  Lw005ProtocolLogger._();

  static bool enabled = true;

  static final Map<int, String> _paramNames = {
    for (final key in Lw005ParamKey.values) key.key: key.name,
  };
  ...
  static void logTx({
    ...
    debugPrint(
      '[LW005 TX] $channel | $operation $param | frame=${_formatHex(payload)}'
  ...
  static void logRx({
    ...
    final parsed = Lw005ProtocolCodec.parseReadResponse(payload);
    ...
    final writeOk = key != null && Lw005ProtocolCodec.isWriteSuccess(payload, key);

    debugPrint(
      '[LW005 RX] $channel | $param$suffix | frame=${_formatHex(payload)}'
  ...
  static void logGattRead({
    ...
    debugPrint(
      '[LW005 GATT READ] $name | raw=${_formatHex(value)} | text="$text"',
    );
  }

  static void logDisconnectNotify(List<int> value) {
    if (!_shouldLog) return;
    debugPrint('[LW005 NOTIFY] disconnect | raw=${_formatHex(value)}');
  }

  static void logError(String message) {
    if (!_shouldLog) return;
    debugPrint('[LW005 ERROR] $message');
  }
  ...
  static String _operationFromPayload(List<int> payload, int key) {
    ...
    if (key == Lw005ProtocolConstants.passwordCmd) {
      return 'PASSWORD';
    }
    return payload[1] == Lw005ProtocolConstants.flagRead ? 'READ' : 'WRITE';
  }
  ...
  static List<int>? _writeDataFromPayload(List<int> payload) {
    ...
    if (payload[1] != Lw005ProtocolConstants.flagWrite) {
      return null;
    }
    if (payload[0] == Lw005ProtocolConstants.headPacket) {
      if (payload.length < 6) {
        return null;
      }
    ...
    if (payload[2] == Lw005ProtocolConstants.passwordCmd) {
