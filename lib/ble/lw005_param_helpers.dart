...
extension Lw005ProtocolApiBatch on Lw005ProtocolApi {
  /// Writes UTC epoch seconds (control 0x53), aligned with native `setTime()`.
  Future<bool> syncTime() async {
    final now = DateTime.now().toUtc();
    final seconds = now.millisecondsSinceEpoch ~/ 1000;
    final bytes = ByteData(4);
    bytes.setInt32(0, seconds, Endian.big);
    return writeParam(Lw005ParamKey.time, bytes.buffer.asUint8List());
  }
}
