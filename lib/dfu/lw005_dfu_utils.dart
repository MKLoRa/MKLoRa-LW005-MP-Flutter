...
Future<String> lw005PrepareDfuFirmwarePath(PlatformFile picked) async {
  final name = picked.name.trim();
  if (name.isEmpty || !name.toLowerCase().endsWith('.zip')) {
    throw Lw005DfuFileException('File error!');
  }
  ...
  final tempDir = Directory.systemTemp.createTempSync('lw005_dfu_');
  final fileName = name.split('/').last.split('\\').last;
  final target = File('${tempDir.path}/$fileName');
  await target.writeAsBytes(bytes, flush: true);
  debugPrint('[LW005 DFU] firmware copied to ${target.path} (${bytes.length} bytes)');
  return target.path;
}

class Lw005DfuFileException implements Exception {
  Lw005DfuFileException(this.message);
