...
import 'device_detail/tabs/ble_tab.dart';
...
class _DeviceDetailPageState extends State<DeviceDetailPage> {
  ...
  Future<void> _loadTabData(int index, {bool showOverlay = true}) async {
    switch (index) {
      ...
      case 2:
        await _bleTabKey.currentState?.load(showOverlay: showOverlay);
      case 3:
        await _deviceTabKey.currentState?.reload(showOverlay: showOverlay);
    }
  }

  Future<void> _onDisconnect(Lw005DisconnectEvent event) async {
    if (_manualBackInProgress ||
        Lw005DfuCoordinator.isUpgrading ||
  ...
  bool get _showSave => _tabIndex == 1 || _tabIndex == 2;
        ...
        body: ColoredBox(
          ...
          children: [
            ...
            BleTab(
              key: _bleTabKey,
              session: widget.session,
              onSaveReady: (save) => _bleSave = save,
            ),
            DeviceTab(
              key: _deviceTabKey,
              session: widget.session,
