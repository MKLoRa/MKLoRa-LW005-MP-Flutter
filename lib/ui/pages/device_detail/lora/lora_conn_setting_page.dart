...
class _LoRaConnSettingPageState extends State<LoRaConnSettingPage> {
  final _state = Lw005LoraConnState();
  ...
  Future<void> _load() async {
    await runWithBleLoading(context, () async {
      ...
      if (!mounted) return;
      final mode = Lw005ParamHelpers.uint8(results[0].data);
      _modeIndex = (mode - 1).clamp(0, 1);
      _devEui.text = Lw005ParamHelpers.bytesToHex(results[1].data);
      _appEui.text = Lw005ParamHelpers.bytesToHex(results[2].data);
      _appKey.text = Lw005ParamHelpers.bytesToHex(results[3].data);
      _devAddr.text = Lw005ParamHelpers.bytesToHex(results[4].data);
      _appSkey.text = Lw005ParamHelpers.bytesToHex(results[5].data);
      _nwkSkey.text = Lw005ParamHelpers.bytesToHex(results[6].data);
  ...
  Future<void> _pickMessageType() async {
    final index = await showBottomPicker(
      context: context,
      options: Lw005OptionLists.payloadTypes,
  ...
  Future<void> _pickMaxRetransmission() async {
    final index = await showBottomPicker(
      context: context,
      options: Lw005OptionLists.retransmissionTimes,
  ...
  Future<void> _pickMode() async {
    final index = await showBottomPicker(
      context: context,
      options: Lw005OptionLists.loraUploadMode,
  ...
  Future<void> _pickRegion() async {
    final index = await showBottomPicker(
      context: context,
      options: Lw005OptionLists.loraRegions,
      selectedIndex: _regionPicker,
    );
    if (index == null) return;
    final region = Lw005LoraConnHelpers.regionFromPicker(index);
    setState(() {
      _regionPicker = index;
      _adr = true;
      Lw005LoraConnHelpers.applyRegion(_state, region, resetValues: true);
    });
  }

  Future<void> _pickCh1() async {
    final options = Lw005LoraConnHelpers.chOptions(_state);
    final index = await showBottomPicker(
      context: context,
      options: options,
  ...
  Future<void> _pickDr() async {
    final options = Lw005LoraConnHelpers.drOptions(_state);
  ...
  Future<bool> _writeAll(Lw005ProtocolApi api) async {
    final region = _state.region;
    if (_modeIndex == 0) {
      if (!await api.writeLoraDevEui(Lw005ParamHelpers.hexToBytes(_devEui.text))) return false;
      if (!await api.writeLoraAppEui(Lw005ParamHelpers.hexToBytes(_appEui.text))) return false;
      if (!await api.writeLoraDevAddr(Lw005ParamHelpers.hexToBytes(_devAddr.text))) return false;
      if (!await api.writeLoraAppSkey(Lw005ParamHelpers.hexToBytes(_appSkey.text))) return false;
      if (!await api.writeLoraNwkSkey(Lw005ParamHelpers.hexToBytes(_nwkSkey.text))) return false;
    } else {
      if (!await api.writeLoraDevEui(Lw005ParamHelpers.hexToBytes(_devEui.text))) return false;
      if (!await api.writeLoraAppEui(Lw005ParamHelpers.hexToBytes(_appEui.text))) return false;
        ...
        children: [
              ...
              children: [
                SettingsLabelRow(
                  ...
                  child: BlueValueButton(
                    text: Lw005OptionLists.loraUploadMode[_modeIndex],
                    onTap: _pickMode,
                  ),
                ),
          ...
          SettingsCard(
            child: SettingsLabelRow(
              label: 'Message Type',
              child: BlueValueButton(
                text: Lw005OptionLists.payloadTypes[_messageTypeIndex],
            ...
            if (_messageTypeIndex == 1)
              SettingsCard(
                child: SettingsNavRow(
                  ...
                  trailing: Lw005OptionLists.retransmissionTimes[_maxRetransIndex],
