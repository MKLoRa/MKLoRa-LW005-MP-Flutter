...
class _LoRaAppSettingPageState extends State<LoRaAppSettingPage> {
  ...
  Future<void> _load() async {
    await runWithBleLoading(context, () async {
      ...
      if (!mounted) return;
      _syncInterval.text = Lw005ParamHelpers.uint8(sync.data).toString();
      _networkCheck.text = Lw005ParamHelpers.uint8(network.data).toString();
    });
  }
  ...
  Widget build(BuildContext context) {
    return DetailScaffold(
      ...
      body: ListView(
        ...
        children: [
          SettingsCard(child: SettingsLabelRow(label: 'Time Sync Interval', child: SettingsTextField(controller: _syncInterval, hint: '0~255', maxLength: 3, suffix: 'H'))),
          SettingsCard(child: SettingsLabelRow(label: 'Network Check Interval', child: SettingsTextField(controller: _networkCheck, hint: '0~255', maxLength: 3, suffix: 'H'))),
        ],
      ),
    );
  }
