...
class BleTabState extends State<BleTab> {
  ...
  Future<void> load({bool showOverlay = true}) async {
    ...
    await runWithBleLoading(
      ...
      () async {
        ...
        if (!mounted) return;
        _advName.text = Lw005ParamHelpers.bytesToString(results[0].data);
        _advInterval.text = Lw005ParamHelpers.uint8(results[1].data).toString();
        _connectable = Lw005ParamHelpers.uint8(results[2].data) == 1;
        _loginPassword = Lw005ParamHelpers.uint8(results[3].data) == 1;
        _loginPasswordOnDevice = _loginPassword;
        final txPower = Lw005ParamHelpers.byte0(results[4].data);
        _txPowerIndex = Lw005OptionLists.txPowerLevels.indexOf(txPower);
  ...
  bool _validate() {
    ...
    return interval != null && interval >= 1 && interval <= 100;
  }
  ...
  Future<bool> _save() async {
    ...
    final writes = <Future<bool>>[
      ...
      api.writeBleTxPower([Lw005OptionLists.txPowerLevels[_txPowerIndex]]),
  ...
  Widget build(BuildContext context) {
    final txPower = Lw005OptionLists.txPowerLevels[_txPowerIndex];
    return ListView(
        ...
        SettingsCard(
          child: Column(
            ...
            children: [
              ...
              const Text(
                Lw005OptionLists.txPowerRangeHint,
                style: TextStyle(
                  fontSize: 12,
                ...
                children: [
                  Expanded(
                    child: Slider(
                      ...
                      min: 0,
                      max: (Lw005OptionLists.txPowerLevels.length - 1).toDouble(),
