...
class _LogDataPageState extends State<LogDataPage> {
  ...
  Future<void> _loadLogs() async {
    final logs = await Lw005DebugLogFile.listLogs(widget.deviceMac);
    if (!mounted) return;
    setState(() {
      _logs = logs;
      _selectedCount = 0;
  ...
  Future<void> _toggleSync() async {
    if (!_syncing) {
      if (_logs.length >= Lw005DebugLogFile.maxFiles) {
        if (!mounted) return;
    ...
    await Lw005DebugLogFile.writeLog(widget.deviceMac, syncTime, _buffer.toString());
    await _loadLogs();
  }
  ...
  Future<void> _deleteSelected() async {
    ...
    if (!ok) return;
    await Lw005DebugLogFile.deleteLogs(_logs);
    await _loadLogs();
  }
